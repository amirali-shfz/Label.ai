...
FROM classification, image, label, (SELECT iid FROM image ORDER BY RANDOM() LIMIT 1) as a
WHERE a.iid = image.iid AND classification.lid = label.lid AND image.iid = classification.iid
...
CREATE VIEW QualifiedUsers AS SELECT * FROM member WHERE mid IN (SELECT mid FROM submission GROUP BY mid HAVING COUNT(*) > 400);
